...
void ClipCohen(Point A, Point B, Point wmin, Point wmax, int g) {
    ...
    while (thoat == 0) {
        if ((Ma(A) | Ma(B)) == 0) thoat = 1;    // A và B n?m trong vùng ch? nh?t
        else if ((Ma(A) & Ma(B)) != 0)        // A-B n?m hoàn toàn bên ngoài vùng ch? nh?t
        {
            thoat = 1; ve = 0;
        } else {
            if (Ma(A) == 0) HoanVi(&A, &B);    // A n?m trong vùng ch? nh?t
            if (A.x == B.x)                    // A và B cùng n?m d?c
            {
...
int main(int argc, char* arg[]) {
    ...
    glutCreateWindow("Thuat Toan Cohen-Sutherland");
